const translations = {
    en: {
        ...
        rule10_1: "First offense: Warning.",
        rule10_2: "Second offense: Revoking some privileges.",
        rule10_3: "Third offense: Kicking out of the team.",
    ...
    ar: {
        ...
        rule10_1: "<strong>المخالفة الأولى:</strong> تنبيه.",
        rule10_2: "<strong>المخالفة الثانية:</strong> سحب بعض الصلاحيات.",
        rule10_3: "<strong>المخالفة الثالثة:</strong> إخراج من التيم.",
    ...
    zh: {
        ...
        rule10_1: "<strong>第一次违规：</strong> 警告。",
        rule10_2: "<strong>第二次违规：</strong> 剥夺部分权限。",
        rule10_3: "<strong>第三次违规：</strong> 踢出战队。",
        imp_title: "⭐ 重要规则",
...
// إدارة فتح وإغلاق النافذة المنبثقة (Modal)
...
// إرسال الاستمارة عبر Discord Webhook
    ...
    formElement.addEventListener("submit", function(e) {
        ...
        const discordMessage = {
            embeds: [{
                ...
                color: 9650346,
                fields: [
                    { name: "👤 الاسم الحقيقي", value: formData.get('real_name') || "غير متوفر", inline: true },
                    { name: "🎮 Minecraft", value: formData.get('minecraft_name') || "غير متوفر", inline: true },
                    { name: "⛏️ سنوات اللعب", value: String(formData.get('years_playing') || "0"), inline: true },
                    { name: "🕒 ساعات اللعب", value: String(formData.get('hours_playing') || "0"), inline: true },
                    { name: "🎂 العمر", value: String(formData.get('age') || "0"), inline: true },
                    { name: "⚔️ المهمة", value: formData.get('role') || "Fighter", inline: true },
        ...
        fetch(webhookURL, {
            ...
            if(response.ok) {
                alert('✅ تم إرسال طلبك بنجاح إلى روم الديسكورد!');
                clanModal.style.display = 'none';
                this.reset();
            } else {
                alert('❌ حدث خطأ أثناء الإرسال، تأكد من البيانات.');
            }
        }).catch(error => {
...
// تشغيل وتوحيد سرعة وحركة جميع سكنات ماينكرفت 3D للمؤسسين والأعضاء
window.addEventListener("DOMContentLoaded", () => {
    // 1. سكن itzDIO
    if (document.getElementById("skin_dio")) {
        let skinViewerDio = new skinview3d.SkinViewer({
            canvas: document.getElementById("skin_dio"),
            width: 200,
            height: 280,
            skin: "./itzdio.png"
        });
        skinViewerDio.controls.enableRotate = true;
        skinViewerDio.controls.enableZoom = false;
        skinViewerDio.animation = new skinview3d.WalkingAnimation();
        skinViewerDio.animation.speed = 0.6;
    }
    ...
    if (document.getElementById("skin_haged")) {
        ...
        skinViewerHaged.controls.enableRotate = true;
        skinViewerHaged.controls.enableZoom = false;
        skinViewerHaged.animation = new skinview3d.WalkingAnimation();
        skinViewerHaged.animation.speed = 0.6;
    }

    // 3. سكن HOMMOS_19 (Member)
    if (document.getElementById("skin_hommos")) {
        let skinViewerHommos = new skinview3d.SkinViewer({
            canvas: document.getElementById("skin_hommos"),
            width: 200,
            height: 280,
            skin: "./HOMMOS_19.png"
        });
        skinViewerHommos.controls.enableRotate = true;
        skinViewerHommos.controls.enableZoom = false;
        skinViewerHommos.animation = new skinview3d.WalkingAnimation();
        skinViewerHommos.animation.speed = 0.6;
    }

    // 4. سكن _D_B_U_ (Member)
    if (document.getElementById("skin_dbu")) {
        ...
        skinViewerDbu.controls.enableRotate = true;
        skinViewerDbu.controls.enableZoom = false;
        skinViewerDbu.animation = new skinview3d.WalkingAnimation();
        skinViewerDbu.animation.speed = 0.6;
    }

    // 5. سكن DIVX_ (Member)
    if (document.getElementById("skin_divx")) {
        let skinViewerDivx = new skinview3d.SkinViewer({
            canvas: document.getElementById("skin_divx"),
            width: 200,
            height: 280,
            skin: "./DIVX_.png"
        });
        skinViewerDivx.controls.enableRotate = true;
        skinViewerDivx.controls.enableZoom = false;
        skinViewerDivx.animation = new skinview3d.WalkingAnimation();
        skinViewerDivx.animation.speed = 0.6;
    }
});